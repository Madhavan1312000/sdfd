...
const searchcomments = () => {
  ...
  axiosInstance.get(`${ipaddress}/searchcoursecomments/${course_id}/${searchvalue}/${user.user_id}/`)
    .then((response) => {
...
axiosInstance.post(`${ipaddress}/coursediscussionpinningview/${user.user_id}/${course_id}/`,formdata)
.then((r)=>{
...
axiosInstance.delete(`${ipaddress}/Unpiningcoursecomments/${user.user_id}/${course_id}/${discid}/`)
.then((r)=>{
  ...
  <p className={x.user_details.profile_pic ==null ? 'd-flex justify-content-center align-items-center bg-warning text-white rounded-circle my-auto' : 'd-none'} style={{height:'40px',width:'40px'}}><span>{x.user_details.nickname.slice(0,1)}</span><span>{x.user_details.nickname.slice(-1)}</span></p>
          ...
          <div className={`col-8 p-0 ${x.pinned_status ? 'col-md-8':'col-md-9'}`} onClick={()=>{
            ...
            <span className='fw-normal ms-2 text-secondary' style={{fontSize:'13px'}}>{x.created_at}</span>
              ...
              <p className='ms-3 ms-sm-0 my-0 d-flex align-items-center' style={{fontSize:'12px',color:'#8587EA'}}>{course_name} <span className={`ms-2 edit ${x.edited ? '':'d-none'}`}>Edited</span></p>
          </div>
          <div className={`col-3 d-flex align-items-center p-0 ${x.pinned_status ? 'col-md-3 justify-content-between':'col-md-2 justify-content-end'}`}>
            <span data-bs-toggle="tooltip" data-bs-placement="top"
         data-bs-custom-class="custom-tooltip"
         data-bs-title="Unfollow" onClick={()=>{
              unpin(x.discid)
            }} style={{cursor:'pointer'}} className={`border p-1 px-2 rounded ${x.pinned_status ? '':'d-none'}`}><i className="fa-solid fa-link-slash d-inline d-md-none"></i> <span className='d-none d-md-inline'>Followed</span></span>
    ...
    <div className="modal-body">
      <div className=' bg-white px-3 pt-2 pb-3 rounded'>
        ...
        {userdetails.nickname!=undefined ? (<p className={userdetails.profile_pic ==null ? 'd-flex justify-content-center align-items-center bg-warning text-white rounded-circle my-auto' : 'd-none'} style={{height:'40px',width:'40px'}}><span>{userdetails.nickname.slice(0,1)}</span><span>{userdetails.nickname.slice(-1)}</span></p>):(<></>)}
          <div className="input-group rounded bg-light border pe-3">
                <input
                  type="text"
                  name="question"
        ...
        {repliesImage4.length > 0 &&
          repliesImage4.map((image, index) => (
            <div key={index} className="image-preview bg-light p-2" style={{position:'relative'}}>
              ...
              <button style={{position:'absolute',top:'-10px',right:'-16px'}} className='btn btn-sm' onClick={() => removeImage4(index)}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-x-circle" viewBox="0 0 16 16">
...
<div className="modal fade" id="replyforreply_modal4" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        ...
        {userdetails.nickname!=undefined ? (<p className={userdetails.profile_pic ==null ? 'd-flex justify-content-center align-items-center bg-warning text-white rounded-circle my-auto' : 'd-none'} style={{height:'40px',width:'40px'}}><span>{userdetails.nickname.slice(0,1)}</span><span>{userdetails.nickname.slice(-1)}</span></p>):(<></>)}
         ...
         data-bs-title="Post" xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none">
          ...
          replies_reply_Image4.map((image, index) => (
            <div key={index} className="image-preview bg-light p-2" style={{position:'relative'}}>
              <img src={URL.createObjectURL(image)} width={50} alt={`Selected Image ${index + 1}`} />
              <button style={{position:'absolute',top:'-10px',right:'-16px'}} className='btn btn-sm' onClick={() => removereply_image4(index)}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-x-circle" viewBox="0 0 16 16">
  <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16"/>
  <path d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708"/>
</svg></button>
            </div>
