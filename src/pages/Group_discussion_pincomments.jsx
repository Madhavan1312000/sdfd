...
const getpinnedcomments=()=>{
    axiosInstance.get(`${ipaddress}/UserPinnedComments/${user.user_id}/${group_id}/`)
    .then((r)=>{
...
function handleLike1(discussion_id) {
 ...
 const editPosts=(discid)=>{
   ...
   if (foundDiscussion) {
     setEditedpost(foundDiscussion.disid.content);
   }
 }
...
// ---------------------------------------------------UNPIN COMMENT-------------------------------------------------------
  ...
  {x.disid.sender.nickname && (<p className={x.disid.sender.profile_pic ==null ? 'd-inine bg-warning text-white p-2 rounded-circle my-auto' : 'd-none'}><span>{x.disid.sender.nickname.slice(0,1)}</span><span>{x.disid.sender.nickname.slice(-1)}</span></p>)}
          ...
          <Link to={`/profile/${x.disid.sender.user_id}`} className='ms-3 ms-sm-0 my-0 fw-medium text-decoration-none text-dark' style={{fontSize:'14px'}}>{x.disid.sender.nickname}
              ...
              <p className='ms-3 ms-sm-0 my-0 d-flex align-items-center' style={{fontSize:'12px',color:'#8587EA'}}>{user.university_name}  <span className={`ms-2 edit ${x.edited ? '':'d-none'}`}>Edited</span></p>
          </div>
         ...
         data-bs-title="Unfollow" style={{cursor:'pointer'}} onClick={()=>{
              unpin(x.disid.id)
            }}><i className="fa-solid fa-link-slash"></i></span>
    ...
    <div className="modal-dialog modal-dialog-centered modal-lg">
      <div className="modal-content">
        ...
        {userdetails.nickname!=undefined ? (<p className={userdetails.profile_pic ==null ? 'd-flex justify-content-center align-items-center bg-warning text-white rounded-circle my-auto' : 'd-none'} style={{height:'40px',width:'40px'}}><span>{userdetails.nickname.slice(0,1)}</span><span>{userdetails.nickname.slice(-1)}</span></p>):(<></>)}
              <div className="input-group pe-3 border bg-light rounded">
                    <input
                      type="text"
                      name="question"
        ...
        {repliesImage1.length > 0 &&
          repliesImage1.map((image, index) => (
            <div key={index} className="image-preview bg-light p-2" style={{position:'relative'}}>
              ...
              <button style={{position:'absolute',top:'-10px',right:'-16px'}} className='btn btn-sm' onClick={() => remove_Image1(index)}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-x-circle" viewBox="0 0 16 16">
